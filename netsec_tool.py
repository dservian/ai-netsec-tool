...
class NetworkSecurityTool:
    def __init__(
        ...
        interface: str,
        model_path: str = None,
        initial_training_packets: int = 10000,
    ):
        ...
        self.model = (
            self.load_model(model_path) if model_path else self.train_new_model()
        )
    ...
    def extract_features(self, packet: scapy.Packet) -> PacketFeatures:
        ...
        return PacketFeatures(
            ...
            http_method=(
                packet[http.HTTPRequest].Method.decode()
    ...
    def train_new_model(self) -> IsolationForest:
        ...
        logging.info(
            f"Capturing {self.initial_training_packets} packets for initial model training..."
        )
        initial_packets = scapy.sniff(
            iface=self.interface, count=self.initial_training_packets
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--interface", required=True, help="Network interface to monitor"
    )
    parser.add_argument(
        "--initial-packets",
        type=int,
        default=10000,
        help="Number of packets for the initial training",
    )
